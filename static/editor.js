...
function newSheet() {
  ...
  persistAll()
  current = sheet.id
  localStorage.setItem("lastSheetId", sheet.id)
  editor.innerHTML = ""
  renderSheets()
  editor.focus()
  updateWordCount()
...
function openSheet(id) {
  ...
  saveEditorSilent()
  current = id
  localStorage.setItem("lastSheetId", id)
  const sheet = sheets.find(s => s.id === id)
  editor.innerHTML = sheet ? (sheet.content || "") : ""
  attachDeleteButtons()
  enableAllTableDragDrop()
...
function deleteSheetById(id) {
  ...
  if (toDelete.has(current)) {
    current = null
    editor.innerHTML = ""
    localStorage.removeItem("lastSheetId")
  }
...
function enableTableDragDrop(table) {
  ...
  function refreshRowHandles() {
    table.querySelectorAll("tr").forEach(tr => {
      if (tr.classList.contains("col-handle-row")) return
      if (tr.querySelector(".row-drag-handle")) return
      tr.draggable = true
      const handle = document.createElement("td")
      handle.className = "row-drag-handle"
  ...
  function refreshColHandles() {
    table.querySelector("tr.col-handle-row")?.remove()
    const firstRow = table.querySelector("tr:not(.col-handle-row)")
    if (!firstRow) return
    ...
    const colCount = firstRow.querySelectorAll("td:not(.row-drag-handle)").length
    for (let i = 0; i < colCount; i++) {
      const th = document.createElement("td")
      th.className = "col-drag-handle"
      th.draggable = true
